...
const MarketRow = ({ item }: { item: CommodityPrice }) => {
  ...
  return (
    <div className="flex items-center justify-between py-3 border-b border-neutral-100 last:border-0 hover:bg-neutral-50/50 transition-colors px-4 group">
      <div className="flex flex-col flex-1">
        <span className="text-[10px] font-black text-neutral-800 uppercase tracking-tight">{item.name}</span>
        <span className="text-[9px] text-neutral-400 font-bold">Confirmed: {item.confirmDate || 'Today'}</span>
      </div>
...
export default function App() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setError("The synthesis engine hit a snag. Please check your API key and try again.");
      setIsLoading(false);
    }
  };
  ...
  const handleSendToSubscribers = async () => {
    ...
    setIsSending(true);
    await sleep(2000); 
    alert(`Edition dispatched to ${subscribers.length} subscribers.`);
    setIsSending(false);
  };
    ...
    <div className="min-h-screen bg-[#fafafa] text-neutral-900 pb-20">
      {showSettings && (
        <div className="fixed inset-0 z-[100] bg-black/40 backdrop-blur-sm flex justify-center items-center p-6" onClick={() => setShowSettings(false)}>
          <div className="w-full max-w-xl bg-white rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
            <div className="p-8 border-b border-neutral-100 flex justify-between items-center bg-neutral-50/50">
              <div className="flex gap-6">
                <button onClick={() => setSettingsTab('email')} className={`text-xs font-black uppercase tracking-widest pb-2 transition-all ${settingsTab === 'email' ? 'text-ag-green border-b-2 border-ag-green' : 'text-neutral-300'}`}>Configuration</button>
            ...
            <div className="p-8 flex-1 overflow-y-auto max-h-[70vh]">
              {settingsTab === 'email' ? (
                ...
                <div className="space-y-6">
                  ...
                  <button onClick={addSubscriber} className="w-full bg-ag-green text-white py-3 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg hover:brightness-110 active:scale-[0.98] transition-all">
      ...
      <main className="max-w-7xl mx-auto px-8 py-10 grid grid-cols-1 lg:grid-cols-2 gap-12">
        <div className="space-y-8">
          <section className="bg-white rounded-[2.5rem] p-8 border border-neutral-200 shadow-sm space-y-6">
            ...
            <textarea value={inputText} onChange={e => setInputText(e.target.value)} placeholder="Paste reports, YouTube transcripts, or industry news..." className="w-full h-44 bg-neutral-50 border-none rounded-3xl p-6 text-sm font-medium focus:ring-2 focus:ring-ag-green shadow-inner resize-none transition-all placeholder:text-neutral-300" />
            <button onClick={() => addToStack('text')} className="w-full bg-neutral-100 text-neutral-500 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-ag-green hover:text-white transition-all">Add Context</button>
          ...
          {curations.length > 0 && (
            <div className="space-y-2">
              <div className="flex justify-between items-center px-4">
                <p className="text-[10px] font-black uppercase text-neutral-400 tracking-widest">Synthesis Queue ({curations.length})</p>
                <button onClick={() => setCurations([])} className="text-[8px] font-black uppercase text-red-400">Clear All</button>
              </div>
              {curations.map(item => (
          ...
          <section className="bg-white rounded-[2.5rem] p-8 border border-neutral-200 shadow-sm space-y-4">
            ...
            <label className="flex items-center gap-3 cursor-pointer group bg-neutral-50 p-4 rounded-2xl border border-neutral-100 hover:bg-neutral-100 transition-all">
              ...
              <div className="flex-1"><span className="block text-[10px] font-black uppercase text-neutral-600">SAFEX Grains & Fibers Charting</span><span className="text-[9px] text-neutral-400">Ground data with live charts and verified pricing dates.</span></div>
            </label>
            <label className="flex items-center gap-3 cursor-pointer group bg-neutral-50 p-4 rounded-2xl border border-neutral-100 hover:bg-neutral-100 transition-all">
              ...
              <div className="flex-1"><span className="block text-[10px] font-black uppercase text-neutral-600">Flash Editorial Visuals</span><span className="text-[9px] text-neutral-400">Powered by Gemini 2.5 Flash Image for high speed.</span></div>
           ...
           <div className="p-12 flex-1 overflow-y-auto custom-scrollbar">
             {isLoading ? (
               <div className="h-full flex flex-col items-center justify-center space-y-8 text-center">
                 ...
                 <p className="text-[10px] font-bold text-ag-gold animate-pulse">Aggregating market intelligence & synthesis...</p>
               </div>
             ) : newsletter ? (
               <div className="animate-in fade-in duration-1000">
                  ...
                  <div className="space-y-16">
                    <div className="bg-white rounded-[2rem] border border-neutral-200 overflow-hidden shadow-sm">
                      <div className="bg-neutral-50/80 px-6 py-3 border-b border-neutral-100 flex justify-between items-center">
                        <div className="flex items-center gap-2">
                           <TrendingUp className="w-4 h-4 text-ag-green" />
                           <h4 className="text-[10px] font-black uppercase tracking-widest text-neutral-900">Market Briefing</h4>
                        </div>
                        {marketAsOf && <span className="text-[9px] font-black text-neutral-400 uppercase">Synced {marketAsOf}</span>}
                      </div>
                      ...
                      <div className="p-3 bg-neutral-50/50 flex justify-center border-t border-neutral-100">
                         <span className="text-[8px] font-black text-neutral-300 uppercase tracking-widest">Grounding with SAFEX & Fiber Global Benchmarks</span>
                      </div>
                    </div>
                          ...
                          {s.imageUrl ? (
                            <img src={s.imageUrl} className="w-full h-full object-cover animate-in fade-in duration-1000" />
                             ...
                             <div className="flex flex-col items-center justify-center space-y-4 text-neutral-300">
                               ...
                               <span className="text-[10px] font-black uppercase tracking-widest">Developing Flash Visual...</span>
                             </div>
                          ) : (
                  ...
                  <div className="mt-24 pt-16 border-t border-neutral-100 space-y-12">
                     <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        <div className="p-8 bg-neutral-50 rounded-[2.5rem] text-center space-y-4 border border-neutral-100 group hover:shadow-xl transition-all">
                           ...
                           <p className="text-[11px] font-medium text-neutral-500">Share The Yield and unlock exclusive producer insights.</p>
                           <button className="text-[9px] font-black uppercase text-ag-green border-b border-ag-green pb-1">Get My Link</button>
                        </div>
                        <div className="p-8 bg-neutral-50 rounded-[2.5rem] text-center space-y-4 border border-neutral-100 group hover:shadow-xl transition-all">
                           <Heart className="w-8 h-8 text-red-400 mx-auto group-hover:scale-110 duration-300" />
                           <h4 className="text-[10px] font-black uppercase tracking-widest text-ag-green">Support Us</h4>
                        ...
                        <div className="p-8 bg-neutral-50 rounded-[2.5rem] text-center space-y-4 border border-neutral-100 group hover:shadow-xl transition-all">
                           ...
                           <h4 className="text-[10px] font-black uppercase tracking-widest text-ag-green">Partnerships</h4>
                           <p className="text-[11px] font-medium text-neutral-500">Advertise to our high-intent network of agricultural pros.</p>
                           <button className="text-[9px] font-black uppercase text-ag-green border-b border-ag-green pb-1">Enquire Now</button>
                        </div>
                     </div>
                  </div>
                  ...
                  <div className="mt-20 py-12 border-t border-neutral-50 text-center space-y-8 pb-20 no-print">
                    ...
                    <div className="flex flex-wrap justify-center gap-4 px-10">
                      <button onClick={handleSendToSubscribers} disabled={isSending} className="flex-1 min-w-[220px] px-8 py-5 rounded-[2rem] bg-ag-green text-white text-[11px] font-black uppercase tracking-widest shadow-xl flex items-center justify-center gap-2 hover:brightness-110 active:scale-95 transition-all">
